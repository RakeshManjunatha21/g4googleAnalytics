...
with tabs[4]:
    ...
    if act is None:
        st.warning("No active_users or GA4_python_output dataset found for Source Attribution.")
    else:
        # Display columns for debugging
        st.write("Columns available in act dataset:", act.columns.tolist())
        if 'sessionMedium' not in act.columns:
            st.warning("'sessionMedium' column not found in act dataset. Please verify column names.")
        else:
